   ...
   //board테이블에 글을 추가(inset문)<=writePro.jsp페이지에서 사
    ...
    throws Exception {
        ...
        try {
            ...
            pstmt.setInt(1, start-1);//mysql에서 첫번째레코드의 시작번호가 0부터 시작함, 따라서 화면표시할 글번호보다 하나 작다
			pstmt.setInt(2, end);//추출할 레코드의 개수
            rs = pstmt.executeQuery();//ResultSet을 반환

            if (rs.next()) {//ResultSet이 레코드를 가짐
                articleList = new ArrayList(end);//몇개의 객체를 저장할지 크기를 지정해서 객체를 생성 =>객체저장소
                do{//134~150라인: ResultSet에 있는 레코드수 만큼 반복수행
			      ...
			      article.setReg_date(rs.getTimestamp("reg_date"));//자바빈객체에 값을 저장
